...
# Pre-requisites:
#
# - must have executed 80_create_snapshot_backup_to_test.sh to snapshot a backup to test
#
# Function:
#
# - Script to create a init.ora file, controlfile and start snapshot database on Data Domain
#
...
then
   ...
   then
      echo " "
      echo "Mounting TEST database using files in directory ${SNAPSHOT_TEST_DIR} "
      echo " "
   else
      echo " "
      echo "${SNAPSHOT_TEST_DIR} does not exist, ensure you have run 80_create_snapshot_backup_to_test.sh"
      echo " "
      exit 1
   fi
else
   echo "Unable to locate snapshot Backup Location ${SNAPSHOT_LOCATION} / Environment not setup properly.. exiting.."
   exit 1
fi
...
*.db_recovery_file_dest='${TEMP_DIR}/'
...
#
# Set ORACLE_SID to TEST_ORACLE_SID and start database within the SNAPSHOT TEST directory
#
...
CREATE CONTROLFILE REUSE SET DATABASE "${TEST_ORACLE_SID}" RESETLOGS NOARCHIVELOG
    ...
    MAXLOGHISTORY 292
LOGFILE
  GROUP 1 '${TEMP_DIR}/${TEST_ORACLE_SID}_redo01.log'  SIZE ${REDO_LOG_SIZE} BLOCKSIZE 512 reuse,
  GROUP 2 '${TEMP_DIR}/${TEST_ORACLE_SID}_redo02.log'  SIZE ${REDO_LOG_SIZE} BLOCKSIZE 512 reuse,
  GROUP 3 '${TEMP_DIR}/${TEST_ORACLE_SID}_redo03.log'  SIZE ${REDO_LOG_SIZE} BLOCKSIZE 512 reuse
DATAFILE
STARTUP-001

#
# Generate list of datafile from ${SNAPSHOT_TEST_DIR} 
#

ls ${SNAPSHOT_TEST_DIR}/*.${DB_FILE_EXTN}* > ${TEMP_DIR}/file_list.out
